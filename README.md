...
3. To make Poptracker auto-update, change the version in `manifest.json`, create the zip for the new release, add a new entry at the **TOP** of `versions.json` (using the filehash of the new zip), and push to Github. For the zip, exclude `versions.json`.